...
class SimpleCommArduino {
 protected:
  ...
  unsigned char bufIn[256];  ///< Buffer temporário para manter os dados de
                             ///< obtidos da porta serial
  unsigned char
      bufOut[256];  ///< Buffer temporário para manter os dados de saída
...
int SimpleCommArduino::write(std::string command) {
  ...
  for (size = 0; size < command.size(); size++) bufOut[size] = command[size];
